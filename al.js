...
function updateNewsDate(){
  ...
  const text = banglaFull + " | " + english + " | " + hijriBangla;

  const pc = document.getElementById("newsContentPC");
  const mobile = document.getElementById("newsContentMobile");

  if(pc) pc.innerHTML = text;
  if(mobile) mobile.innerHTML = text;
}
